...
def to_iso_datetime(raw_date_str):
    try:
        dt = datetime.strptime(raw_date_str, "%d/%m/%Y %I:%M %p")
        return dt.isoformat()
    except:
        return None
...
def extract_from_pdf(pdf_bytes):
    ...
    raw_recorded_at = get_val("Recorded at", r"Recorded at:\s+(.*)")
    return {
        "Dataset name": get_val("Dataset name", r"Dataset name:\s+(.*)"),
        "Recorded at": to_iso_datetime(raw_recorded_at),
...
def parse_files():
    try:
        data = request.json
        pdf_bytes = download_file(data.get("pdf_url"))
        image_bytes = download_file(data.get("screenshot_url"))
        dataset_name = data.get("dataset_name")

        pdf_data = extract_from_pdf(pdf_bytes)
        image_data = extract_from_image(image_bytes, dataset_name)
        return jsonify({**pdf_data, **image_data})
    except Exception as e:
